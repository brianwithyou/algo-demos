...
//        System.out.printf("board[%s][%s]: %s, index: %s \n", startX, startY, board[startX][startY], word.substring(0, index + 1));
        if (inArea(startX, startY) && board[startX][startY] == word.charAt(index) && !visited[startX][startY]) {
    ...
    public static void main(String[] args) {
        ...
        boolean exist1 = new Solution1().exist(data1, "ABCCED");
        char[][] data0 = new char[][]{
                {'A', 'B'},
                {'C', 'D'}
        };
        boolean exist0 = new Solution1().exist(data0, "ABCD");

        char[][] data2 = new char[][]{
                {'A'},
        };
        boolean exist2 = new Solution1().exist(data2, "A");
